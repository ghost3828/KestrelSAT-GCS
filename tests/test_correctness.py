...
root, app = make_app()
app.delimiter_var.set("tab")
app.update_delimiter()
check(app.delimiter == "\t", "delimiter is a real tab, not %r" % app.delimiter)
...
for cycle in range(5):
    for n in range(3):
        app.update_plot_channels({"Ch%d_%d" % (cycle, i): float(i) for i in range(4)}, n)
    root.update()
    app.clear_plot_data()
    root.update()
