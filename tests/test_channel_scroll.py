...
app.notebook.select(app.plots[0].frame)  # Plot tab - widgets aren't mapped/sized until selected
root.update()
root.update_idletasks()

# 1. ordering: plot_control_frame's ancestor must appear before channel_frame
# among plot_frame's children.
btn_bar = app.plots[0].show_plot_btn.master.master  # btn_inner -> btn_bar
assert btn_bar in app.plots[0].frame.winfo_children()
...
assert app.plots[0].plot_scroll_canvas not in (btn_bar,), "buttons must not be inside the scroll area"
...
for n in range(3):
    app.plots[0].update_plot_channels(data, n)
root.update()
root.update_idletasks()

assert len(app.plots[0].channels) == NUM_CHANNELS
canvas_h = app.plots[0].plot_scroll_canvas.winfo_height()
...
bbox = app.plots[0].plot_scroll_canvas.bbox("all")
...
app.plots[0].plot_scroll_canvas.yview_moveto(0)
root.update()
top_y = app.plots[0].plot_scroll_canvas.yview()[0]
app.plots[0].plot_scroll_canvas.yview_moveto(1)
root.update()
bottom_y = app.plots[0].plot_scroll_canvas.yview()[0]
...
app.plots[0]._on_plot_mousewheel(FakeEvent())
root.update()
after_wheel = app.plots[0].plot_scroll_canvas.yview()[0]
...
last_row_btn = app.plots[0].channels[last_channel].color_btn
canvas_top = app.plots[0].plot_scroll_canvas.winfo_rooty()
canvas_bottom = canvas_top + app.plots[0].plot_scroll_canvas.winfo_height()
...
root.update()
rows = app.plots[0].channel_rows_frame.winfo_children()
assert len(rows) == 1 and isinstance(rows[0], __import__("tkinter").ttk.Label)
assert rows[0].cget("text") == "No channels detected"
assert app.plots[0].ylabel_entry.winfo_exists()
assert app.plots[0].plot_scroll_canvas.yview()[0] == 0.0
...
for n in range(3):
    app.plots[0].update_plot_channels(data, n + 100)
root.update()
for name in sg.THEME_ORDER:
    ...
    root.update()
    c = sg.THEMES[name]
    assert str(app.plots[0].plot_scroll_canvas.cget("bg")) == c["surface"], (name, app.plots[0].plot_scroll_canvas.cget("bg"))
